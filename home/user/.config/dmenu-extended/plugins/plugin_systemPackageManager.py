...
class extension(dmenu_extended.dmenu):
    ...
    def __init__(self):
        ...
        if os.path.exists('/usr/bin/apt-get'):
            ...
            self.command_listInstalled = 'cd /var/db/pkg/ && ls -d */* > ' + dmenu_extended.path_cache + '/tmp.txt'
            self.command_listAvailable = 'emerge --search "" > ' + dmenu_extended.path_cache + '/tmp.txt'
	...
	self.availablePackages_portage_prep()
	packages = self.command_output('cat ' + dmenu_extended.path_cache + '/tmp.txt')
	os.system('rm '  + dmenu_extended.path_cache + '/tmp*')
        return packages

    def availablePackages_portage_prep(self):
	os.system(self.command_listAvailable)
	os.system('grep "*  " ' + dmenu_extended.path_cache + '/tmp.txt > ' + dmenu_extended.path_cache + '/tmp2.txt')
	os.system('cut -c 4- ' + dmenu_extended.path_cache + '/tmp2.txt > ' + dmenu_extended.path_cache + '/tmp.txt')
	os.system('sed "s/\[ Masked \]//g" ' + dmenu_extended.path_cache + '/tmp.txt > ' + dmenu_extended.path_cache + '/tmp2.txt')
	os.system()
	os.system('rm '  + dmenu_extended.path_cache + '/tmp2.txt')
